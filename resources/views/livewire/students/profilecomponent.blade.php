<div>
    ...
    <x-inputs.group label="Preferred Pronoun" for="pronoun_id">
        <x-inputs.select label="" :options="$pronouns" for="pronoun_id" currentvalue="{{ $student->person->pronoun_id }}"/>
    </x-inputs.group>
    ...
    <x-inputs.group label="Shirt size" for="shirtsize">
        <x-inputs.select label="" :options="$shirtsizes" for="shirtsize_id" currentvalue="{{ $student->shirtsize_id }}"/>
    </x-inputs.group>
    ...
    <footer class="mt-4 bg-gray-200 flex justify-end space-x-2 p-2">
        ...
        <x-buttons.button wire:click="save" >Update {{ ucwords($student->person->fullname) }}</x-buttons.button>
    </footer>
</div>
